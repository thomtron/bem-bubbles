...
class PLYLoader extends Loader {
	...
	parse( data ) {
		...
		function postProcess( buffer ) {
			...
			if ( buffer.potentials.length > 0 ) {

				geometry.setAttribute( 'potential', new Float32BufferAttribute( buffer.potentials, 1 ) );

			}

            if ( buffer.normal_derivatives.length > 0 ) {

				geometry.setAttribute( 'normal_derivative', new Float32BufferAttribute( buffer.normal_derivatives, 1 ) );
		...
		function handleElement( buffer, elementName, element ) {
            ...
            const attrW = findAttrName( [ 'w' ]);
            const attrPHI = findAttrName( [ 'phi' ] );
			const attrPSI = findAttrName( [ 'psi' ] );
			...
			if ( elementName === 'vertex' ) {
				...
				if ( attrPHI != null ) {
					buffer.potentials.push( element[ attrPHI ] );
				}

                if ( attrPSI != null ) {
					buffer.normal_derivatives.push( element[ attrPSI ] );
				}

                if ( attrW != null && attrPHI == null) {
                    buffer.potentials.push( element[ attrW ] );
                }
